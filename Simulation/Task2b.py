...
def sysCall_actuation():
    ...
    if message == sim.message_keypress:
        if data[0] == 2007:  # Up arrow key
            self.desired_position += 0.3  # Increase position change per press (adjust for faster response)
        elif data[0] == 2008:  # Down arrow key
            self.desired_position -= 0.3  # Increase position change per press (adjust for faster response)
        elif data[0] == 2009:  # Left arrow key
            self.desired_heading -= 7  # Increase heading change per press (adjust for faster response)
